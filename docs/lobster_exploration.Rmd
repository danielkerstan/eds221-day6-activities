---
title: "lobster exploration"
author: "Daniel Kerstan"
date: "8/16/2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(tidyverse)
library(here)
library(janitor)
```

```{r}
lobster_abundance <- read_csv(here("data", "Lobster_Abundance_All_Years_20210412.csv"), na = c("-99999", "")) %>% 
  clean_names() %>% 
  uncount(count) #made them all show up instead of just a count
```

```{r}
lobster_summary <- lobster_abundance %>% 
  group_by(site, year) %>% 
  summarize(mean_carapace_length = mean(size_mm, na.rm = TRUE),
            n = n())

summary_graph <- ggplot(lobster_summary, aes(x = year, y = n)) + geom_point() + facet_wrap("site")

summary_graph

```

Find the proportion of legal lobsters at each site for 2020

```{r}
lobster_2020 <- lobster_abundance %>% 
  filter(year == 2020)
```

```{r}
lobster_2020_legal <- lobster_2020 %>% 
  mutate(legal = case_when(
    size_mm >= 79.76 ~ "yes",
    size_mm <79.76 ~ "NO"
  )) 

lobster_summary_2020 <- lobster_2020_legal %>% 
  group_by(site, legal) %>% 
  summarize(number = n())
```

```{r}
legal_graph <- ggplot(lobster_summary_2020, aes(x = site, y = number, fill = legal)) +
  geom_col(position = "fill")

legal_graph
```


Random lobster wrangling

filter() practice
```{r}
ex_a <- lobster_abundance %>% 
  filter(site %in% c("IVEE", "CARP", "NAPL"))
unique(lobster_abundance$site)
unique(ex_a$site)
```
```{r}
ex_b <- lobster_abundance %>% 
  filter(month == 8)
unique(ex_b$month)
```

```{r}
ex_c <- lobster_abundance %>% 
  filter(site == "AQUE" | size_mm > 70)
```

group_by() %>% summarize() practice

```{r}
ex_e <- lobster_abundance %>% 
  group_by(site) %>% 
  summarize(mean = mean(size_mm, na.rm = TRUE),
            sd = sd(size_mm, na.rm = TRUE))

ex_f <- lobster_abundance %>% 
  group_by(site, month) %>% 
  summarize(maximum = max(size_mm))
```

```{r}
ex_g <- lobster_abundance %>% 
  mutate(size_cm = size_mm / 10)

ex_h <- lobster_abundance %>% 
  mutate(site = tolower(site))

ex_i <- lobster_abundance %>% 
  mutate(area = as.character(area))
class(ex_i$area)


ex_j <- lobster_abundance %>% 
  mutate(size_bin = case_when(
    size_mm <= 70 ~ "small",
    size_mm > 70 ~ "large"
  ))

ex_k <- lobster_abundance %>% 
  mutate(designation = case_when(
    site %in% c("IVEE", "NAPL") ~ "MPA",
    TRUE ~ "not MPA"
  ))
```


